                ...
                if (rs.next()) {
                    try {
                        metrics = new BundlerJobMetrics.BundlerJobMetricsBuilder()
                                ...
                                .archiveType(ArchiveType.fromString(
                                        rs.getString("ARCHIVE_TYPE")))
                                .elapsedTime(rs.getLong("ELAPSED_TIME"))
                                .jobID(rs.getString("JOB_ID"))
                                .jobState(JobStateType.fromString(
                        ...
                        LOGGER.warn("Unexpected UnknownArchiveTypeException "
                                + "encountered while loading job metrics "
                                + "data from the data source.  Job ID [ "
                                + rs.getString("JOB_ID")
                                + " ] has a type of [ "
                                + rs.getString("ARCHIVE_TYPE")
                                + " ].  Error message => [ "
                    ...
                    catch (UnknownJobStateTypeException ujste) {
                        LOGGER.warn("Unexpected UnknownJobStateTypeException "
                                + "encountered while loading job metrics "
                                + "data from the data source.  Job ID [ "
                                + rs.getString("JOB_ID")
                                + " ] has a job state of [ "

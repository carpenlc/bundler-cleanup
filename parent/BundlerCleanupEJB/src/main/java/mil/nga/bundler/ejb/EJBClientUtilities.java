...
public class EJBClientUtilities {
    ...
    /**
     * Utility method used to look up the DatasourceCleanupService interface.  
     * This method is only called by the web tier.
     * 
     * @return The DatasourceCleanupService interface, or null if we couldn't 
     * look it up.
     */
    public DatasourceCleanupService getDatasourceCleanupService() 
            throws EJBLookupException {
        
        DatasourceCleanupService service = null;
        Object               ejb     = getEJB(DatasourceCleanupService.class);
        
        if (ejb != null) {
            if (ejb instanceof mil.nga.bundler.ejb.DatasourceCleanupService) {
                service = (DatasourceCleanupService)ejb;
            }
            else {
                throw new EJBLookupException("Unable to look up EJB [ "
                        + getJNDIName(DatasourceCleanupService.class)
                        + " ] returned reference was the wrong type.  "
                        + "Type returned [ "
                        + ejb.getClass().getCanonicalName()
                        + " ].",
    ...
    /**
     * Utility method used to look up the DiskCleanupService interface.  
     * This method is only called by the web tier.
     * 
     * @return The DiskCleanupService interface, or null if we couldn't 
     * look it up.
     */
    public DiskCleanupService getDiskCleanupService() 
            throws EJBLookupException {
        
        DiskCleanupService service = null;
        Object               ejb     = getEJB(DiskCleanupService.class);
        
        if (ejb != null) {
            if (ejb instanceof mil.nga.bundler.ejb.DiskCleanupService) {
                service = (DiskCleanupService)ejb;
            }
            else {
                throw new EJBLookupException("Unable to look up EJB [ "
                        + getJNDIName(DiskCleanupService.class)
                        + " ] returned reference was the wrong type.  "
                        + "Type returned [ "
                        + ejb.getClass().getCanonicalName()
                        + " ].",
                        DiskCleanupService.class.getName());
            }
        }
        else {
            throw new EJBLookupException(
